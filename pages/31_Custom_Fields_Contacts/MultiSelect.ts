...
export class MultiSelectField extends CommonSteps {
    ...
    async deleteCustomField() {
        ...
        const isTextPresent = await this.page.getByText('Test Contact Multi Select').count();
        if (isTextPresent === 0) {
            console.log('Test Contact Multi Select does not exist, aborting function.');
            return;
        }
        await this.waitForTextStrict('Test Contact Multi Select');
        await this.clickOnTextStrict('Test Contact Multi Select');
    ...
    async createNewMultiSelectCustomColumnAndAddIttoGridColumns() {
        ...
        await this.selectingDropdownValuebyLabel(ContactLocators.dataTypeLocator, 'Multi Select');
        await this.waitForPlaceholder(ContactLocators.fieldNamePlaceholder);
        await this.fillingPlaceholder(ContactLocators.fieldNamePlaceholder, 'Test Contact Multi Select');
        await this.waitForPlaceholder(ContactLocators.descriptionPlaceholder);
        await this.fillingPlaceholder(ContactLocators.descriptionPlaceholder, 'Description for Custom Contact Multi Select Field');
