...
export class SingleSelectField extends CommonSteps {
    ...
    async deleteCustomField() {
        ...
        const isTextPresent = await this.page.getByText('Test Company Single Select').count();
        if (isTextPresent === 0) {
            console.log('Test Company Single Select does not exist, aborting function.');
            return;
        }
        await this.waitForTextStrict('Test Company Single Select');
        await this.clickOnTextStrict('Test Company Single Select');
